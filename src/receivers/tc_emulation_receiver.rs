...
pub fn factory(
    ...
    let path = _config.get("path").and_then(|value| value.as_str()).unwrap_or("trace.tc_emulation.txt").to_string();
    let interval = _config.get("interval").and_then(|value| value.as_u64()).unwrap_or(1000000);
...
impl AbstractReceiver for TcEmulationReceiver {
    ...
    fn _receive_entry(&mut self, entry: Entry) {
        match entry {
          Entry::Event { timestamp, kind: EventKind::SyncStart { .. } } => {
          ...
          Entry::Event { timestamp, kind } => {
            if timestamp / self.interval != self.curr_tc {
              ...
              let slack = (timestamp / self.interval - self.curr_tc) * self.interval;
